...
int nfvfs_open(struct nfvfs *nfvfs, const char *path, int flags, int mode)
{
    int fd;
    int i;
    int fentry;
...
int nfvfs_lseek(struct nfvfs *nfvfs, int fd, int offset, int whence)
{
    ...
    ret = nfvfs->super.op.lseek(fd, offset, whence);
    if (ret < 0)
        return ret;
...
{
    return nfvfs->super.op.readdir(fd, buf);
}
...
int nfvfs_list(struct nfvfs *nfvfs, int fd, int (*action)(const char *name, void *data), void *data)
{
    ...
    int ret = 0;
    while (nfvfs_readdir(nfvfs, fd, &dentry) == 0) {
