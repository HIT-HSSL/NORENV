    ...
    u32_t flag)
{
  ...
  // printf("%d %d %d %d\n", len / 256, needed_pages, free_pages, fs->stats_p_deleted);
  // printf("len %d, needed pages %d, free pages %d, allocated %d, deleted %d\r\n",
  //  len, needed_pages, free_pages, fs->stats_p_allocated, fs->stats_p_deleted);
  ...
  if ((s32_t)needed_pages > (s32_t)(free_pages + fs->stats_p_deleted))
  {
    SPIFFS_GC_DBG("gc_check: full freeblk:"_SPIPRIi
                  ...
                  "\n",
                  fs->free_blocks, needed_pages, free_pages, fs->stats_p_deleted);
    // printf("len , page size : %d %d", len, free_pages * (s32_t)SPIFFS_DATA_PAGE_SIZE(fs));
    return SPIFFS_ERR_FULL;
  }
  ...
  // Debug
  // printf("%d, %d, %d, %d, %d, %d\n", fs->stats_p_allocated + fs->stats_p_deleted, fs->stats_p_deleted, fs->free_blocks, free_pages, tries, res);
  return res;
}
