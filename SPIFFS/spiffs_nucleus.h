...
#define SPIFFS_CHECK_MAGIC_POSSIBLE(fs) \
  ((SPIFFS_OBJ_LOOKUP_MAX_ENTRIES(fs) % (SPIFFS_CFG_LOG_PAGE_SZ(fs) / sizeof(spiffs_obj_id))) * sizeof(spiffs_obj_id) <= (SPIFFS_CFG_LOG_PAGE_SZ(fs) - sizeof(spiffs_obj_id) * 2))
...
typedef struct SPIFFS_PACKED
{
  ...
  // flags
  u8_t flags;
} __attribute__((__packed__)) spiffs_page_header;
