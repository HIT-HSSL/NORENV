    ...
    spiffs_page_ix *objix_hdr_pix)
{
  ...
  res = spiffs_gc_check(fs, SPIFFS_DATA_PAGE_SIZE(fs), 1);
  SPIFFS_CHECK_RES(res);
...
s32_t spiffs_object_append(spiffs_fd *fd, u32_t offset, u8_t *data, u32_t len)
{
  ...
  res = spiffs_gc_check(fs, len + SPIFFS_DATA_PAGE_SIZE(fs), 1); // add an extra page of data worth for meta
  if (res != SPIFFS_OK)
  {
...
s32_t spiffs_object_modify(spiffs_fd *fd, u32_t offset, u8_t *data, u32_t len)
{
  ...
  res = spiffs_gc_check(fs, len + SPIFFS_DATA_PAGE_SIZE(fs), 1);
  SPIFFS_CHECK_RES(res);
    ...
    u8_t remove_full)
{
  ...
  if (remove_full == 0)
  {
    res = spiffs_gc_check(fs, SPIFFS_DATA_PAGE_SIZE(fs) * 2, 1);
    SPIFFS_CHECK_RES(res);
  }
    ...
    u8_t *dst)
{
  ...
  while (cur_offset < offset + len)
  {
    ...
    if (fd->ix_map && data_spix >= fd->ix_map->start_spix && data_spix <= fd->ix_map->end_spix && fd->ix_map->map_buf[data_spix - fd->ix_map->start_spix])
    {
