...
// lfs_block_t¾ÍÊÇuint32_t,¼õÒ»±íÊ¾É¶
...
static int lfs_init(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    if (lfs->cfg->read_buffer) {
				// ¹ÒÔØ¹ý³ÌÖÐ×Ô¼ºÉêÇëÄÚ´æÇé¿ö
        lfs->rcache.buffer = lfs->cfg->read_buffer;
    } else {
        lfs->rcache.buffer = lfs_malloc(lfs->cfg->cache_size);
        if (!lfs->rcache.buffer) {
						//----------------------------ADD----------------------
						//¶¯Ì¬ÄÚ´æ£¬¼´ÎÄ¼þÏµÍ³×Ô¶¯ÉêÇë
//						printf("¼Ä¼Ä¼Ä¼Ä¼Ä¼Ä\r\n");
            err = LFS_ERR_NOMEM;
            goto cleanup;
        }
...
static int lfs_rawmount(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    lfs_mdir_t dir = {.tail = {0, 1}}; // ¸ødirµÄ²ÎÊýtail¸³Öµ {0, 1}
    lfs_block_t cycle = 0;
		//¿´tailÁ½¸ö¿éÖµÊÇ·ñ±íÊ¾¿Õ£¬µ«Ç°Ãæ²»ÊÇÒÑ¾­¸³ÖµÁËÂð£¿£¿£¿£¿
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        if (tag && !lfs_tag_isdelete(tag)) {
            ...
            lfs_superblock_fromle32(&superblock); //³¬¼¶¿éµÄÊý¾Ý´Ó´æ´¢½éÖÊ»ñµÃºóÓÉÐ¡¶Ë±äÎª±¾»úÀàÐÍ
    ...
    lfs->gdisk = lfs->gstate; //gdiskºÍgstateÒ»Ñù£¬ÄÇÓÐÉ¶ÓÃ£¿£¿£¿£¿

    // setup free lookahead, to distribute allocations uniformly across
    // boots, we start the allocator at a random location
		// off±íÊ¾µÚ¼¸¸ö¿é£¬¹ÒÔØÊ±µÄ³õÊ¼Î»ÖÃ
...
#ifdef LFS_THREADSAFE  //Ïß³Ì°²È«£¬ÐèÒª×Ô¼ºÌá¹©º¯Êý±£Ö¤
#define LFS_LOCK(cfg)   cfg->lock(cfg)
#define LFS_UNLOCK(cfg) cfg->unlock(cfg)
#else
#define LFS_LOCK(cfg)   ((void)cfg, 0) //ÔÚ¸ÉÂï£¿£¿£¿£¿£¿
