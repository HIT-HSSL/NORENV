...
static int lfs_file_rawopen(lfs_t *lfs, lfs_file_t *file,
        const char *path, int flags) {
    static const struct lfs_file_config defaults = {0}; //?????全为0吗
...
static int lfs_rawmount(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    lfs_mdir_t dir = {.tail = {0, 1}}; // 给dir的参数tail赋值 {0, 1}
    lfs_block_t cycle = 0;
		//看tail两个块值是否表示空，但前面不是已经赋值了吗？？？？
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        if (tag && !lfs_tag_isdelete(tag)) {
            ...
            lfs_superblock_fromle32(&superblock); //超级块的数据从存储介质获得后由小端变为本机类型
    ...
    lfs->gdisk = lfs->gstate; //gdisk和gstate一样，那有啥用？？？？
...
#ifdef LFS_THREADSAFE  //线程安全，需要自己提供函数保证
#define LFS_LOCK(cfg)   cfg->lock(cfg)
#define LFS_UNLOCK(cfg) cfg->unlock(cfg)
#else
#define LFS_LOCK(cfg)   ((void)cfg, 0) //在干嘛？？？？？
